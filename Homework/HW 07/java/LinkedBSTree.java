...
public class LinkedBSTree<T extends Comparable<T>> {
  ...
  {
    //readaptation of inorder
    return maxArea(root).getArea();
  }
  private Shape maxArea(Node aNode) {
    Shape holder = new Shape();
    if(aNode == null)
      return holder;
    maxArea(aNode.leftChild);//Go left
    System.out.println(aNode.data);
    maxArea(aNode.rightChild);//go right
    return null;
    
  }
  
 
  public void deleteAreasGreater(double aArea)
  {
    deleteAreasGreater(root, aArea);
  }
  private void deleteAreasGreater(Node aNode, double aArea) {
    if(aNode == null)
      return;
    if(aNode.data.getArea()>aArea)
      return;
    deleteAreasGreater(aNode.leftChild,aArea);//Go left
    System.out.println(aNode.data);//Access the node
    deleteAreasGreater(aNode.rightChild,aArea);
  } 
  */
}
